export const articlesData = [
  {
    id: 'first-job',
    title: 'Первая работа или стажировка',
    articles: [
      {
        id: 1,
        title: 'Почему работодатели не любят новичков — и что на самом деле скрывается за фразой «нужен опыт»',
        category: 'career',
        link: './articles_pages/pochemu-rabotodateli-ne-lyubyat-novichkov.html',
        image: 'pochemu-rabotodateli-ne-lyubyat-novichkov'
      },
      {
        id: 2,
        title: 'Как устроена адаптация на первой работе: почему первые недели такие тяжелые',
        category: 'life',
        link: './articles_pages/kak-ustroena-adaptaciya-na-pervoj-rabote.html',
        image: 'kak-ustroena-adaptaciya-na-pervoj-rabote'
      },
      {
        id: 3,
        title: 'Испытательный срок: не дай себя обмануть',
        category: 'career',
        link: './articles_pages/ispytatelnyj-srok-ne-daj-sebya-obmanut.html',
        image: 'ispytatelnyj-srok-ne-daj-sebya-obmanut'
      }
    ]
  },
  {
    id: 'moving',
    title: 'Переезд в другой город',
    articles: [
      {
        id: 4,
        title: 'Почему найти квартиру в новом городе — это квест с багами и без сохранений',
        category: 'home',
        link: './articles_pages/kvartira-v-novom-gorode.html',
        image: 'kvartira-v-novom-gorode'
      },
      {
        id: 5,
        title: 'Первый месяц в новом городе: на чем вы незаметно потеряете деньги',
        category: 'finance',
        link: './articles_pages/pervyj-mesyac-v-novom-gorode.html',
        image: 'pervyj-mesyac-v-novom-gorode'
      },
      {
        id: 6,
        title: 'Прописка, военкомат, полис: зачем они вообще нужны и что будет, если забить',
        category: 'docs',
        link: './articles_pages/propiska-voenkomat-polis.html',
        image: 'propiska-voenkomat-polis'
      }
    ]
  },
  {
    id: 'car-owners',
    title: 'Для тех, кто купил машину (или только собирается)',
    articles: [
      {
        id: 7,
        title: 'Почему новички за рулем чаще попадают в ДТП — что говорит статистика и психология',
        category: 'life',
        link: './articles_pages/pochemu-novichki-za-rulem-chashche-popadayut-v-dtp.html',
        image: 'pochemu-novichki-za-rulem-chashche-popadayut-v-dtp'
      },
      {
        id: 8,
        title: 'Первая машина: как не попасть на деньги, на кого смотреть и где подвох',
        category: 'home',
        link: './articles_pages/pervaya-mashina-kak-ne-popast-na-dengi.html',
        image: 'pervaya-mashina-kak-ne-popast-na-dengi'
      },
      {
        id: 9,
        title: 'Страховка, налог, ТО: как устроены обязательные платежи автовладельца',
        category: 'docs',
        link: './articles_pages/strahovka-nalog-to-obyazatelnye-platezhi-avtovladelca.html',
        image: 'strahovka-nalog-to-obyazatelnye-platezhi-avtovladelca'
      }
    ]
  },
  {
    id: 'first-alone',
    title: 'Впервые живешь один или одна',
    articles: [
      {
        id: 10,
        title: 'Почему быт жрет время и как устроена «ментальная нагрузка» домашних дел',
        category: 'life',
        link: './articles_pages/pochemu-byt-zhret-vremya-i-kak-ustroena-mentalnaya-nagruzka.html',
        image: 'pochemu-byt-zhret-vremya-i-kak-ustroena-mentalnaya-nagruzka'
      },
      {
        id: 11,
        title: 'Коммуналка: как накручивают, почему мы платим за соседа и что такое ОДН',
        category: 'docs',
        link: './articles_pages/kommunalka-kak-nakruchivayut-pochemu-my-platim-za-soseda-i-chto-takoe-odn.html',
        image: 'kommunalka-kak-nakruchivayut-pochemu-my-platim-za-soseda-i-chto-takoe-odn'
      },
      {
        id: 12,
        title: 'Минимальный набор для жизни: без чего реально нельзя, а без чего — можно',
        category: 'home',
        link: './articles_pages/minimalnyj-nabor-dlya-zhizni.html',
        image: 'minimalnyj-nabor-dlya-zhizni'
      }
    ]
  },
  {
    id: 'uni-live',
    title: 'Выживание в университете',
    articles: [
      {
        id: 13,
        title: 'Почему прокрастинация побеждает в сессию — и как работает стресс на экзаменах',
        category: 'life',
        link: './articles_pages/pochemu-prokrastinaciya-pobezhdaet-v-sessiyu.html',
        image: 'pochemu-prokrastinaciya-pobezhdaet-v-sessiyu'
      },
      {
        id: 14,
        title: 'Как подготовиться к защите диплома и не поседеть за месяц до',
        category: 'life',
        link: './articles_pages/kak-podgotovitsya-k-zashchite-diploma-i-ne-posedet-za-mesyac-do.html',
        image: 'kak-podgotovitsya-k-zashchite-diploma-i-ne-posedet-za-mesyac-do'
      },
      {
        id: 15,
        title: '«Работа и учёба одновременно: как не вылететь из вуза и не уволиться через месяц»',
        category: 'career',
        link: './articles_pages/rabota-i-uchyoba-odnovremenno.html',
        image: 'rabota-i-uchyoba-odnovremenno'
      }
    ]
  },
  {
    id: 'going-abroad',
    title: 'Первая самостоятельная поездка за границу',
    articles: [
      {
        id: 16,
        title: 'Как спланировать первую поездку за границу: от выбора страны до посадки в самолёт',
        category: 'life',
        link: './articles_pages/kak-splanirovat-pervuyu-poezdku-za-granicu.html',
        image: 'kak-splanirovat-pervuyu-poezdku-za-granicu'
      },
      {
        id: 17,
        title: 'Как не потерять деньги на картах и обменниках в другой стране',
        category: 'life',
        link: './articles_pages/kak-ne-poteryat-dengi-na-kartah-i-obmennikah-v-drugoj-strane.html',
        image: 'kak-ne-poteryat-dengi-na-kartah-i-obmennikah-v-drugoj-strane'
      },
      {
        id: 18,
        title: 'Просроченная виза и потерянный загранпаспорт: инструкция по выживанию в чужой стране',
        category: 'career',
        link: './articles_pages/prosrochennaya-viza-i-poteryannyj-zagranpasport.html',
        image: 'prosrochennaya-viza-i-poteryannyj-zagranpasport'
      }
    ]
  },
  {
    id: 'lost-docs',
    title: 'Потеря документов',
    articles: [
      {
        id: 19,
        title: 'Как устроен черный рынок документов — почему не надо постить паспорт в интернет',
        category: 'life',
        link: './articles_pages/kak-ustroen-chernyj-rynok-dokumentov.html',
        image: 'kak-ustroen-chernyj-rynok-dokumentov'
      },
      {
        id: 20,
        title: 'Что происходит с вашими данными, если телефон украли (и при чем тут SIM-карта)',
        category: 'life',
        link: './articles_pages/chto-proiskhodit-s-vashimi-dannymi-esli-telefon-ukrali.html',
        image: 'chto-proiskhodit-s-vashimi-dannymi-esli-telefon-ukrali'
      },
      {
        id: 21,
        title: 'Квест по восстановлению паспорта',
        category: 'docs',
        link: './articles_pages/kvest-po-vosstanovleniyu-pasporta.html',
        image: 'kvest-po-vosstanovleniyu-pasporta'
      }
    ]
  },
  {
    id: 'getting-fired',
    title: 'Увольнение (по своей воле или не очень)',
    articles: [
      {
        id: 22,
        title: 'Почему нас увольняют: юридические и психологические причины',
        category: 'career',
        link: './articles_pages/pochemu-nas-uvolnyayut-yuridicheskie-i-psihologicheskie-prichiny.html',
        image: 'pochemu-nas-uvolnyayut-yuridicheskie-i-psihologicheskie-prichiny'
      },
      {
        id: 23,
        title: 'Что такое «выходное пособие» и кто его получает',
        category: 'finance',
        link: './articles_pages/chto-takoe-vyhodnoe-posobie-i-kto-ego-poluchaet.html',
        image: 'chto-takoe-vyhodnoe-posobie-i-kto-ego-poluchaet'
      },
      {
        id: 24,
        title: 'Сокращение, ликвидация, по собственному: чем различаются ситуации с точки зрения закона',
        category: 'career',
        link: './articles_pages/sokrashchenie-likvidaciya-po-sobstvennomu.html',
        image: 'sokrashchenie-likvidaciya-po-sobstvennomu'
      }
    ]
  },
  {
    id: 'rent-apartment',
    title: 'Съемная квартира — права и обязанности',
    articles: [
      {
        id: 25,
        title: 'Кто должен платить за ремонт, если сломалась техника — арендатор или собственник?',
        category: 'finance',
        link: './articles_pages/kto-dolzhen-platit-za-remont-esli-slomalas-tekhnika.html',
        image: 'kto-dolzhen-platit-za-remont-esli-slomalas-tekhnika'
      },
      {
        id: 26,
        title: 'Почему собственники не возвращают депозит — 5 самых частых схем',
        category: 'finance',
        link: './articles_pages/pochemu-sobstvenniki-ne-vozvrashchayut-depozit-5-samyh-chastyh-skhem.html',
        image: 'pochemu-sobstvenniki-ne-vozvrashchayut-depozit-5-samyh-chastyh-skhem'
      },
      {
        id: 27,
        title: 'Что такое договор найма и почему «договоренности на словах» всегда заканчиваются плохо',
        category: 'docs',
        link: './articles_pages/chto-takoe-dogovor-najma.html',
        image: 'chto-takoe-dogovor-najma'
      }
    ]
  },
  {
    id: 'freelance',
    title: 'Выход на фриланс / самозанятость',
    articles: [
      {
        id: 28,
        title: 'Почему фриланс кажется легким, но выживают не все',
        category: 'career',
        link: './articles_pages/nestabilnost-na-frilanse.html',
        image: 'nestabilnost-na-frilanse'
      },
      {
        id: 29,
        title: 'Как устроены налоги для самозанятых: что будет, если не платить, и кто за вами следит',
        category: 'finance',
        link: './articles_pages/nalogi-dlya-samozanyatyh.html',
        image: 'nalogi-dlya-samozanyatyh'
      },
      {
        id: 30,
        title: 'Фриланс vs офис: что теряешь и что получаешь на удаленке',
        category: 'docs',
        link: './articles_pages/frilans-vs-ofis.html',
        image: 'frilans-vs-ofis'
      }
    ]
  },
  {
    id: 'freelance',
    title: 'Когда родители болеют и нуждаются в помощи',
    articles: [
      {
        id: 31,
        title: 'Почему пожилые скрывают свои болезни — стыд, страх и как устроено старение',
        category: 'health',
        link: './articles_pages/kak-ustroeno-starenie.html',
        image: 'kak-ustroeno-starenie'
      },
      {
        id: 32,
        title: 'Что дает государство тем, кто ухаживает за родственником (пособия, стаж, льготы)',
        category: 'docs',
        link: './articles_pages/posobiya-po-uhodu.html',
        image: 'posobiya-po-uhodu'
      },
      {
        id: 33,
        title: 'Как устроена бесплатная помощь пенсионерам: соцработники, сиделки, паллиатив',
        category: 'health',
        link: './articles_pages/besplatnaya-pomoshch-pensioneram.html',
        image: 'besplatnaya-pomoshch-pensioneram'
      }
    ]
  },
  {
    id: 'first-property',
    title: 'Покупка первого жилья (или ипотека)',
    articles: [
      {
        id: 34,
        title: 'Почему ипотека на 20 лет — это не страшно, если понимать математику',
        category: 'finance',
        link: './articles_pages/ipoteka.html',
        image: 'ipoteka'
      },
      {
        id: 35,
        title: 'Скрытые расходы при покупке квартиры: оценка, нотариус, страховка — на чем теряют деньги',
        category: 'finance',
        link: './articles_pages/skrytye-raskhody-pri-pokupke-kvartiry.html',
        image: 'skrytye-raskhody-pri-pokupke-kvartiry'
      },
      {
        id: 36,
        title: 'Новостройка vs вторичка: как устроены риски каждой',
        category: 'health',
        link: './articles_pages/novostrojka-vs-vtorichka.html',
        image: 'novostrojka-vs-vtorichka'
      }
    ]
  },
  {
    id: 'relationship-finance',
    title: 'Отношения и общие финансы',
    articles: [
      {
        id: 37,
        title: 'Раздельный, общий или частично общий бюджет: какие схемы реально работают в парах',
        category: 'finance',
        link: './articles_pages/razdelnyj-ili-obshchij-byudzhet.html',
        image: 'razdelnyj-ili-obshchij-byudzhet'
      },
      {
        id: 38,
        title: 'Почему деньги — главная причина разводов и как устроена «финансовая неверность»',
        category: 'finance',
        link: './articles_pages/prichina-razvodov.html',
        image: 'prichina-razvodov'
      },
      {
        id: 39,
        title: 'Кредиты и долги в паре: кто платит, если расстались (как устроено юридически)',
        category: 'finance',
        link: './articles_pages/oplata-dolgov-posle-rasstavaniya.html',
        image: 'oplata-dolgov-posle-rasstavaniya'
      }
    ]
  },
  {
    id: 'taxes',
    title: 'Налоги и взносы — когда ты уже работаешь',
    articles: [
      {
        id: 40,
        title: 'Самозанятость, ИП, физлицо: чем отличаются и кто кому сколько платит',
        category: 'finance',
        link: './articles_pages/samozanyatost-ip-fizlico.html',
        image: 'samozanyatost-ip-fizlico'
      },
      {
        id: 41,
        title: 'Почему налоговая присылает письма с долгами даже тем, кто все заплатил — как устроены ошибки и сверки',
        category: 'docs',
        link: './articles_pages/oshibki-nalogovoj.html',
        image: 'oshibki-nalogovoj'
      },
      {
        id: 42,
        title: 'Куда на самом деле идут 13% НДФЛ — и почему мы почти не видим, куда они ушли',
        category: 'finance',
        link: './articles_pages/kak-rabotaet-ndfl.html',
        image: 'kak-rabotaet-ndfl'
      }
    ]
  },
  {
    id: 'visiting-a-doctor',
    title: 'Ходим к врачу по-взрослому',
    articles: [
      {
        id: 43,
        title: 'Почему врачи могут быть грубыми — как устроена система ОМС и перегрузка в поликлиниках',
        category: 'docs',
        link: './articles_pages/kak-ustroena-sistema-oms.html',
        image: 'kak-ustroena-sistema-oms'
      },
      {
        id: 44,
        title: 'Что такое информированное согласие и почему его надо читать',
        category: 'docs',
        link: './articles_pages/informirovannoe-soglasie.html',
        image: 'informirovannoe-soglasie'
      },
      {
        id: 45,
        title: 'Платная vs бесплатная медицина: где реально разница, а где просто дорогой ремонт',
        category: 'health',
        link: './articles_pages/platnaya-vs-besplatnaya-medicina.html',
        image: 'platnaya-vs-besplatnaya-medicina'
      }
    ]
  },
  {
    id: 'first-credit',
    title: 'Первый кредит или рассрочка',
    articles: [
      {
        id: 46,
        title: 'Рассрочка vs кредит: в чем разница и почему «0%» не равно «бесплатно»',
        category: 'finance',
        link: './articles_pages/rassrochka-vs-kredit.html',
        image: 'rassrochka-vs-kredit'
      },
      {
        id: 47,
        title: 'Кредитная история: как за ней следят и почему из-за глупости в 18 лет могут не дать ипотеку в 30',
        category: 'docs',
        link: './articles_pages/kreditnaya-istoriya.html',
        image: 'kreditnaya-istoriya'
      },
      {
        id: 48,
        title: 'Почему банки одобряют кредиты тем, кто и так при деньгах — а бедным отказывают',
        category: 'finance',
        link: './articles_pages/komu-banki-odobryayut-kredity.html',
        image: 'komu-banki-odobryayut-kredity'
      }
    ]
  },
]